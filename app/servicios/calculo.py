...
# Guardamos el constructor original
_original_init = fills.PatternFill.__init__

def safe_init(self, *args, **kwargs):
    # Eliminamos 'extLst' de los argumentos si está presente para evitar el error
    kwargs.pop('extLst', None)
    _original_init(self, *args, **kwargs)

# Reemplazamos el constructor por nuestra versión segura
fills.PatternFill.__init__ = safe_init
# --- FIN DEL PARCHE ---
...
class DataContext:
    """
    Carga estudiantes.csv, resultados.csv y avernia_ues.xlsx
    ...
    def _load_carreras(path: str) -> pd.DataFrame:
        # se lee directamente el Excel avernia_ues.xlsx
        df = pd.read_excel(
            path,
            engine="openpyxl",
        )
    ...
    def _get_pes_estudiante(self, codigo_estudiante: str) -> Decimal:
        """
    ...
    def calcular_puntaje_postulacion(self, codigo_estudiante: str, codigo_carrera: int) -> float:
        """
        ...
        """
        # Validar estudiante
        if codigo_estudiante not in self.df_estudiantes.index:
            raise EstudianteNoEncontrado(codigo_estudiante)

        # Validar carrera
        if codigo_carrera not in self.df_carreras.index:
            raise CarreraNoEncontrada(codigo_carrera)

        carrera = self.df_carreras.loc[codigo_carrera]
